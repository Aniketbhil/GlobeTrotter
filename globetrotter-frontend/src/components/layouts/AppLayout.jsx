...
export const AppLayout = () => {
  ...
  return (
    // FIXED: Changed min-h-screen to h-screen overflow-hidden to lock the page height
    <div className="h-screen overflow-hidden bg-background flex flex-col md:flex-row">
      
      {/* Mobile Top Navbar */}
      <div className="md:hidden flex items-center justify-between p-4 bg-surface border-b border-border-default z-30 shrink-0">
        {/* LOGO & TITLE */}
      ...
      <aside className={`
        fixed inset-y-0 left-0 z-40 w-64 bg-surface border-r border-border-default transform transition-transform duration-300 ease-in-out
        md:relative md:translate-x-0 flex flex-col shrink-0
        ${isMobileMenuOpen ? 'translate-x-0 top-18.25 h-[calc(100vh-73px)]' : '-translate-x-full md:top-0 md:h-screen'}
      `}>
        {/* LOGO & TITLE (Desktop) */}
        <div className="hidden md:flex p-6 items-center gap-3 shrink-0">
          <img src={logo} alt="GlobeTrotter Logo" className="h-8 w-auto" onError={(e) => { e.target.style.display = 'none'; }} />
          <span className="font-manrope font-bold text-2xl text-text-primary tracking-tight">GlobeTrotter</span>
        </div>
        ...
        <div className="p-4 border-t border-border-subtle shrink-0">
          {user && (
            <div className="flex items-center gap-3 px-4 py-3 mb-2 bg-surface-muted rounded-xl">
              <div className="w-8 h-8 rounded-full bg-primary text-text-on-primary flex items-center justify-center font-bold shrink-0">
          ...
          <Link to="/profile" onClick={() => setIsMobileMenuOpen(false)} className="flex items-center gap-3 px-4 py-3 rounded-xl text-text-secondary hover:bg-surface-hover hover:text-text-primary transition-colors">
            <Settings size={20} />
            Settings
          </Link>
      ...
      {isMobileMenuOpen && (
        <div 
          className="md:hidden fixed inset-0 bg-background/50 backdrop-blur-sm z-20 top-18.25"
          onClick={() => setIsMobileMenuOpen(false)}
        />
      )}
